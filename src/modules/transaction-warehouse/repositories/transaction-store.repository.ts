...
import { TransactionWarehouseEntity } from "../entities/transaction-warehouse.entity";
import { FindTransactionWarehouseDto } from "../dto/find-transaction-warehouse.dto";

@Injectable()
export class TransactionWarehouseRepository extends Repository<TransactionWarehouseEntity> {
    constructor(private readonly dataSource: DataSource) {
        super(TransactionWarehouseEntity, dataSource.createEntityManager());
    }


        async findAll(dto: FindTransactionWarehouseDto, userPayload: IJwtPayload, manager?: EntityManager) {
            const repo = manager ? manager.getRepository(TransactionWarehouseEntity) : this;
            const queryBuilder = repo.createQueryBuilder('transaction-warehouse');
            // queryBuilder.select([
            //   'warehouse.id as id',
            // ])
            ...
            queryBuilder.orderBy('transaction-warehouse.created_at', 'DESC')
